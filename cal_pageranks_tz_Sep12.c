...
int myanalysis(double res[10], int snap, int idAl, int ntot, char Labels[Size][2], double Coords[Size][3], double clustersize, int graphAlO, double cutoffAlO, int graphOO, double cutoffOO, int graphOH, double cutoffOH, double dampingf, int Ntot, int sindex[Size], int getforce, double target, double kbias);
// the fermi function
...
int main(int argc, char *argv[])
{
	...
	double fxyz[Size][3];   // this is the force on each atom, 2017.09.18
...
//printf(" %d %d %f\n",k,i,temp);
	...
	//		printf(" snap %d CN %d %f CS %d nAl %d nO %d nH %d\n",num,cn,fcn,nAl*3-nO*2+nH,nAl,nO,nH);
		...
		for(k=1;k<=nAl;k++)
		{
			...
			test=myanalysis(results,num,k,nn,slabel,sxyz,clustersize,graphAlO,cutoffAlO,graphOO,cutoffOO,graphOH,cutoffOH,damping,ntot,sindex,getforce,target,kbias);
